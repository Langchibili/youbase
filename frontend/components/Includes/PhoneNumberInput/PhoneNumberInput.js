...
export default function PhoneNumberInput({ setCountryCode, setPhoneNumber }) {
  const [selectedCode, setSelectedCode] = useState('260') // Default to Zambia
  const [phone, setPhone] = useState('')

  const handleCountryCodeChange = (event) => {
    const newCode = event.target.value
    setSelectedCode(newCode)
    setCountryCode(newCode) // Call the provided prop function
  }

  const handlePhoneNumberChange = (event) => {
    const newPhone = event.target.value
    setPhone(newPhone)
    setPhoneNumber(newPhone) // Call the provided prop function
  }

  useEffect(()=>{ // set the default country code, zambia's 260 for now
    setCountryCode('260') // Call the provided prop function
  },[])
