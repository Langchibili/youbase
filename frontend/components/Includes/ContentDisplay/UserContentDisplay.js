...
export default function UserContentDisplay(props) {
  ...
  useEffect(() => {
     const runAsyncFeedGet = async ()=>{ // user feed
            const userData = await getUserById(props.user.id,"posts.user,posts.featuredImages,posts.media")
  ...
  if (props.contentToView === 'portrait-videos' || props.contentToView === 'portrait-images') { // filter out portrait content
    console.log('feed portrait content 1', portraitContent)
  ...
  return ( // display landscape content
    <div>
      {posts.map((post) => {
        ...
        if(props.postTypeToDisplay){ // if at all user wants specific content
            if(props.postTypeToDisplay !== post.type){
                return
            }
        }
